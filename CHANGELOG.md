...
Light mode is now the default. The tool previously followed the operating system prefers-color-scheme setting; it now opens in light mode for everyone. Dark mode is applied only if the user explicitly toggled to it in a previous session, and that saved preference is still honored.

## v2.9.15

Replaced the empty-state waveform with a nicer one: a traveling audio-packet burst (a gaussian envelope sweeping across a speech-like carrier) with a soft glow underlay, faint grid ticks, and a leading dot — more fitting for a VoIP signal tool. It is now fully theme-aware: the scope canvas, trace, and grid all read from the CLIR color tokens, so it renders as a teal trace on a light card in light mode and a bright teal trace on the dark card in dark mode, matching the rest of the UI.

## v2.9.14

Fixed the empty-state scope waveform not animating. It was being suppressed when the browser/OS had "reduce motion" enabled, and after the palette change its dark canvas and line colors had gone stale (teal-on-near-white in dark mode). The gentle decorative waveform now animates for everyone, draws on a fixed dark scope with a bright teal trace that reads in both light and dark mode, and restarts when you return to the upload screen via New file.

## v2.9.13

Added a "How to read this tab" help guide at the top of the Legend tab. It explains the highlight convention with a small colour key: brightly highlighted statuses and reasons are the ones that actually occurred in the capture you loaded (what matters right now), while dimmed "(not in this capture)" entries are reference-only and can be ignored for the current file. Includes a plain-English rule of thumb so users know what to focus on.

## v2.9.12

Made the primary actions prominent: "Copy summary" is now a filled teal (signal) button and "New file" a filled blue (accent) button, standing out from the ghost-style controls. All button text meets WCAG AA in both light and dark mode.
